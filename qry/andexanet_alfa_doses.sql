WITH DOSES AS (
	...
	FROM 
		...
		ORDERS
	WHERE
		CLINICAL_EVENT.EVENT_CD = 2841975793 -- coagulation factor Xa
		AND CLINICAL_EVENT.EVENT_END_DT_TM BETWEEN
			DECODE(
				...
				'User-defined', pi_to_gmt(
					TO_DATE(
						@Prompt('Enter begin date', 'D', , mono, free, persistent, {'07/01/2023 00:00:00'}, User:80),
			...
			AND DECODE(
				...
				'User-defined', pi_to_gmt(
					TO_DATE(
						@Prompt('Enter end date', 'D', , mono, free, persistent, {'7/01/2024 00:00:00'}, User:81),
						pi_get_dm_info_char_gen('Date Format Mask|FT','PI EXP|Systems Configuration|Date Format Mask')
					) - 1/86400,
					'America/Chicago'
				)
