...
pts <- read_excel("U:/Data/admin_requests/burn_cse/external/patient_list.xlsx", skip = 2) %>%
...
data_dir <- "U:/Data/admin_requests/burn_cse/raw"

raw_opioids <- get_data(data_dir, "opioids_followup")
...
df_mme_drip <- df_drips %>%
    drip_runtime(vars(fin), id = fin, med_datetime = event_datetime) %>%
...
# raw_pca <- get_data(data_dir, "pca_followup") %>%
...
data_mme <- df_mme_meds %>%
    ...
    group_by(fin) %>%
    mutate(
        total_mme = sum(mme_iv, mme_drips, na.rm = TRUE),
        total_mme_no_tramadol = sum(mme_no_tramdol, mme_drips, na.rm = TRUE))

openxlsx::write.xlsx(data_mme, "U:/Data/admin_requests/burn_cse/final/2020-08_burn-pts_morph-equivalents.xlsx")
