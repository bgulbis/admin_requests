WITH DOSES AS (
	...
	SELECT DISTINCT
		DOSES.*,
		ORDER_DETAIL.OE_FIELD_DISPLAY_VALUE AS PRN_REASON
	FROM
		DOSES,
		ORDER_DETAIL
	WHERE
		DOSES.ORIG_ORDER_ID = ORDER_DETAIL.ORDER_ID(+)
		AND ORDER_DETAIL.OE_FIELD_MEANING_ID(+) = 142 -- PRNREASON
)
...
SELECT DISTINCT
	...
	pi_get_cv_display(PERSON.SEX_CD) AS GENDER,
...
FROM
	...
	PERSON
WHERE
	ORD_DETAILS.ENCNTR_ID = ENCOUNTER.ENCNTR_ID
	AND ORD_DETAILS.PERSON_ID = PERSON.PERSON_ID
	AND ORD_DETAILS.ENCNTR_ID = ENCNTR_ALIAS.ENCNTR_ID
